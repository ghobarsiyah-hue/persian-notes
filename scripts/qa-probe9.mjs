/* PM selection state while typing in title + DOMObserver behavior */
...
const selInfo = await page.evaluate(() => {
  ...
  const node = ed.state.doc.nodeAt(s.from);
  return { from: s.from, to: s.to, nodeTypeAtFrom: node?.type.name ?? null, isNodeSel: !!(s.node) };
});
console.log('PM selection after title click:', JSON.stringify(selInfo));
await browser.close();
